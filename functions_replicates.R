...
minus_loglikeli_my_new <- function(thetas, datumT, NNarrayT, m, Lambda, negativ = TRUE) {
  ...
  for(r in 1:R){
    ...
    loglikelihood <- loglikelihood  + 6 * log(b[1]) - a_post * log(b[1] + crossprod(datum[, 1], solve(Lambda, datum[, 1]))/2)
